# MON's Recommended Tools

[简体中文](README.md) | [English](README.en.md)

A curated list of software I use regularly and recommend. The tools are organized into only four broad categories. Each entry includes an introduction, key features, advantages, and an official link; open-source projects also include their GitHub repository.

> Download software only from official websites or project repositories. Use proxy, tunneling, download, and streaming tools in accordance with local laws, service terms, and content licenses.

## Contents

- [Recording, Creation, and Documents](#recording-creation-and-documents)
- [System, Productivity, and Development](#system-productivity-and-development)
- [Downloads and Networking](#downloads-and-networking)
- [Media and Entertainment](#media-and-entertainment)

## Recording, Creation, and Documents

### OBS Studio

OBS Studio is a free, open-source, cross-platform application for screen recording and live streaming, suitable for game streams, tutorials, presentations, and multi-camera production.

- **Features:** Scene and source composition, window/screen/game capture, webcam and audio mixing, live streaming, local recording, filters, transitions, and plugins.
- **Advantages:** Free with no watermark; highly configurable; available for Windows, macOS, and Linux; backed by a mature community and plugin ecosystem.
- **Website:** [obsproject.com](https://obsproject.com/)
- **GitHub:** [obsproject/obs-studio](https://github.com/obsproject/obs-studio)

### Bandicam

Bandicam is a commercial screen recorder focused on Windows desktop and game capture, ideal for users who want polished recordings with minimal setup.

- **Features:** Region, game, webcam, and external-device recording; microphone mixing; on-screen drawing; mouse effects; scheduled recording; and screenshots.
- **Advantages:** Easy to learn; strong hardware-encoding support; balances quality, performance, and file size through real-time compression.
- **Website:** [bandicam.com](https://www.bandicam.com/)

### Typora

Typora is a Markdown editor built around a focused, single-pane live-preview experience instead of separate source and preview panes.

- **Features:** Live Markdown rendering, tables, code blocks, math, Mermaid diagrams, outlines, file trees, themes, and multi-format export.
- **Advantages:** Clean and distraction-free; intuitive editing of complex Markdown; polished CJK typography; excellent for notes and documentation.
- **Website:** [typora.io](https://typora.io/)

### Foxit PDF Reader

Foxit PDF Reader is a cross-platform PDF reader and annotation tool for everyday reading, review, form filling, and document signing.

- **Features:** PDF viewing and printing, highlights and comments, form filling, handwritten or digital signatures, sharing, and selected AI tools.
- **Advantages:** Fast startup and rendering; more capable than a basic viewer; familiar Office-style interface; available on desktop, mobile, and web.
- **Website:** [foxit.com/pdf-reader](https://www.foxit.com/pdf-reader/)

### Arctime

Arctime is a cross-platform subtitle-production application for video creators, covering transcription, timing, styling, and subtitle rendering.

- **Features:** Audio/video waveform timing, subtitle editing, AI speech recognition, automatic timing, styling, translation assistance, and rendering.
- **Advantages:** Intuitive timeline workflow; keeps subtitle tasks in one application; friendly to Chinese-language production; supports major desktop systems.
- **Website:** [arctime.cn](https://arctime.cn/)

## System, Productivity, and Development

### Geek Uninstaller

Geek Uninstaller is a lightweight Windows utility for removing desktop programs, Microsoft Store apps, and leftover files or entries.

- **Features:** Standard uninstall, leftover scanning, forced removal, registry/install-folder access, and software-list export.
- **Advantages:** Portable single-file application; starts and scans quickly; more convenient than the built-in Windows list for troublesome software.
- **Website:** [geekuninstaller.com](https://geekuninstaller.com/)

### DeskBox

DeskBox is a free, open-source Windows desktop organizer that uses native-feeling widgets to manage real folders and everyday information.

- **Features:** Desktop file zones, automatic organization, folder mapping, file stacks, to-dos, quick notes, search, weather, and media controls.
- **Advantages:** Keeps files in normal folders; fits the Windows 10/11 visual style; local-first; supports x64 and ARM64.
- **Website:** [deskbox.fun](https://deskbox.fun/)
- **GitHub:** [Tianyu199509/DeskBox](https://github.com/Tianyu199509/DeskBox)

### Escrcpy

Escrcpy is a graphical Android device manager built on scrcpy, making it easier to display and control phones from a computer.

- **Features:** USB/wireless ADB, low-latency mirroring and control, recording, screenshots, multi-device management, file/APK operations, mapping, and automation.
- **Advantages:** Wraps scrcpy's performance in an approachable interface; cross-platform; requires no root access; useful for demos, testing, and device management.
- **Website:** [Project homepage](https://github.com/viarotel-org/escrcpy)
- **GitHub:** [viarotel-org/escrcpy](https://github.com/viarotel-org/escrcpy)

### Everything

Everything is a local filename search utility for Windows that builds a fast index to locate files and folders almost instantly.

- **Features:** Real-time filename search, filtering and sorting, advanced query syntax, regular expressions, bookmarks, and result export.
- **Advantages:** Extremely fast indexing and results; small installer; low resource usage; ideal for disks containing very large numbers of files.
- **Website:** [voidtools.com](https://www.voidtools.com/)

### PowerToys

Microsoft PowerToys is an open-source suite of Windows enhancements maintained by Microsoft.

- **Features:** FancyZones, quick launching, keyboard remapping, bulk rename, color picker, text extraction, image resizing, always-on-top windows, and more.
- **Advantages:** Covers many frequent needs in one application; integrates naturally with Windows; modules can be enabled individually; actively maintained.
- **Website:** [Microsoft Learn: PowerToys](https://learn.microsoft.com/windows/powertoys/)
- **GitHub:** [microsoft/PowerToys](https://github.com/microsoft/PowerToys)

### WizTree

WizTree is a Windows disk-space analyzer designed to quickly identify the files and folders using the most storage.

- **Features:** Folder tree and treemap views, size and count sorting, file-type statistics, filtering, result export, and cleanup assistance.
- **Advantages:** Exceptionally fast NTFS scanning; makes disk usage easy to understand; portable edition available; excellent before cleanup.
- **Website:** [diskanalyzer.com](https://diskanalyzer.com/)

### Huorong Internet Security

Huorong is a Windows endpoint-security application combining malware protection, network defenses, and useful system-management tools.

- **Features:** Malware scanning, real-time protection, firewall, vulnerability fixes, startup management, junk cleanup, and pop-up blocking.
- **Advantages:** Clean interface; relatively quiet during daily use; combines security and maintenance; offers detailed protection and rule settings.
- **Website:** [huorong.cn](https://www.huorong.cn/)

### DeepSeek Harness

DeepSeek Harness is DeepSeek's local-first, extensible, open-source coding agent and agent runtime, currently in developer preview.

- **Features:** Code reading and editing, shell and tool use, web search, skills, subagents, sandboxes, session tracing, and plugin composition.
- **Advantages:** Its “everything is a plugin” design is extensible; traces can be resumed, forked, searched, and replayed; multiple runtime modes support experimentation.
- **Website:** [deepseek.com/harness](https://www.deepseek.com/harness/)
- **GitHub:** [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness)

## Downloads and Networking

### Clash Verge Rev

Clash Verge Rev is an open-source, cross-platform proxy client based on Mihomo and Tauri, continuing the original Clash Verge project.

- **Features:** Subscription/profile management, system proxy, TUN mode, visual node/rule editing, merge and script enhancements, and WebDAV backup.
- **Advantages:** Supports Windows, macOS, and Linux; offers a clear interface and powerful configuration; follows community development closely.
- **Website:** [clashverge.dev](https://www.clashverge.dev/)
- **GitHub:** [clash-verge-rev/clash-verge-rev](https://github.com/clash-verge-rev/clash-verge-rev)

### Neat Download Manager

Neat Download Manager is a free download manager for Windows and macOS emphasizing lightweight design, segmented downloads, and browser integration.

- **Features:** HTTP/HTTPS/FTP, dynamic segmentation, pause/resume, categories, bandwidth limits, proxies, browser extensions, and HLS merging.
- **Advantages:** Small installer and simple interface; free and cross-platform; uses available bandwidth efficiently and recovers interrupted downloads.
- **Website:** [neatdownloadmanager.com](https://neatdownloadmanager.com/)

### Internet Download Manager

Internet Download Manager (IDM) is a mature commercial Windows download manager known for browser integration and reliable segmented downloading.

- **Features:** Browser interception, dynamic segmentation, resume, batch tasks, site grabbing, queues, scheduling, and bandwidth limits.
- **Advantages:** Broad browser compatibility; reliable interruption recovery; complete queue and scheduling tools; strong website compatibility.
- **Website:** [internetdownloadmanager.com](https://www.internetdownloadmanager.com/)

### qBittorrent

qBittorrent is a free, open-source, ad-free BitTorrent client with a complete and consistent cross-platform experience.

- **Features:** Torrent and magnet downloads, DHT/PEX/LSD, RSS automation, bandwidth scheduling, priorities, search, torrent creation, and Web UI.
- **Advantages:** Free with no ads; consistent across major desktop platforms; reasonable resource usage; comprehensive task and network controls.
- **Website:** [qbittorrent.org](https://www.qbittorrent.org/)
- **GitHub:** [qbittorrent/qBittorrent](https://github.com/qbittorrent/qBittorrent)

### SakuraFrp Launcher

SakuraFrp Launcher is the client for the SakuraFrp tunneling service, exposing internal game servers, websites, or remote desktops through public relays.

- **Features:** Tunnel creation and control, TCP/UDP/HTTP(S) forwarding, auto-start, logs, remote management, and multi-platform/container deployment.
- **Advantages:** Friendly Chinese panel and documentation; works without a public IP; centralizes relay selection and tunnel management.
- **Website:** [natfrp.com](https://www.natfrp.com/)
- **GitHub:** [natfrp](https://github.com/natfrp) (documentation and selected platform projects; Windows 2.x source is archived and the 3.x core is no longer open source)

## Media and Entertainment

### foobar2000

foobar2000 is an audio player known for local-library management and deep customization, especially useful for large personal music collections.

- **Features:** Mainstream audio formats, gapless playback, ReplayGain, tag editing, CD ripping, conversion, internet radio, DSP, and components.
- **Advantages:** Lightweight and stable; excellent metadata management; highly customizable interface, shortcuts, and playback chain; rich ecosystem.
- **Website:** [foobar2000.org](https://www.foobar2000.org/)

### Honeyview

Honeyview is a lightweight image viewer from Bandisoft, particularly good at browsing images, animations, and comics inside archives.

- **Features:** Multi-format viewing, slideshows, EXIF/GPS data, rotation and zoom, direct archive viewing, and comic-reading mode.
- **Advantages:** Starts quickly and is easy to use; browses images without extraction; convenient for comics and large image sets.
- **Website:** [Bandisoft Honeyview](https://www.bandisoft.com/honeyview/) (development has ended; BandiView is the official successor)

### PotPlayer (64-bit)

PotPlayer is a Windows media player with broad format compatibility and highly detailed playback controls.

- **Features:** Local/network media, built-in codecs, hardware acceleration, subtitle synchronization, playlists, processing, capture, and 360° video.
- **Advantages:** Excellent format support; efficient high-resolution playback; extensive subtitle and filter controls; supports deep customization.
- **Website:** [potplayer.daum.net](https://potplayer.daum.net/)

### Spotify

Spotify is a cross-platform streaming service for music, podcasts, and audiobooks in supported markets.

- **Features:** Online playback, personal/collaborative playlists, cross-device control, recommendations, Premium offline downloads, and yearly summaries.
- **Advantages:** Broad catalog; mature recommendation and playlist ecosystem; seamless switching between devices; strong sharing features.
- **Website:** [spotify.com](https://www.spotify.com/)

### Kazumi

Kazumi is an open-source anime discovery and streaming application driven by user-supplied collection rules.

- **Features:** Search and schedules, rule sharing, multiple sources, danmaku, watchlists, history, speed control, downloads, DLNA, and Anime4K.
- **Advantages:** Supports Android, Windows, macOS, Linux, and iOS; flexible rules; capable integrated player; active community.
- **Website:** [kazumi.app](https://kazumi.app/)
- **GitHub:** [Predidit/Kazumi](https://github.com/Predidit/Kazumi)

### Lossless Scaling

Lossless Scaling is a Windows scaling and frame-generation utility distributed through Steam for games and windowed applications.

- **Features:** LSFG frame generation, LS1/FSR/NIS scaling, integer scaling, Anime4K, game profiles, automatic activation, and dual-GPU offloading.
- **Advantages:** Broad compatibility with older games, emulators, and frame-limited titles; no native game integration required; flexible quality/performance tradeoffs.
- **Website:** [Lossless Scaling on Steam](https://store.steampowered.com/app/993090/Lossless_Scaling/)

## About This List

- Recommendations reflect personal use cases; features, pricing, and licenses may change.
- Official GitHub repositories are included for open-source projects; closed-source tools link only to official websites.
- Issues and pull requests are welcome for experience reports, corrections, and additional recommendations.